...
% else:
    Your submission "${reviewable_title}", submitted to ${provider.name} has ${'not been accepted and will be made private and not discoverable by others. You may edit the preprint and contact the moderator at '+ provider_support_email +' to resubmit.' if is_rejected else 'been accepted by the moderator and remains discoverable to others. '} ${'The moderator has also provided a comment that is only visible to contributors of the preprint, and not to others. ' if notify_comment else ''}
% endif
